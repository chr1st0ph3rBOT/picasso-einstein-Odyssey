...
class AppX_GUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(main_frame, text="Artist X", font=self.font_title, bg=self.bg_color, fg=self.fg_color).pack(pady=(0, 20))
        
        self.load_button = tk.Button(main_frame, text="Artist X 모델 불러오기 (.pth)", command=self.load_model, 
        ...
        tk.Label(main_frame, text="결과:", font=self.font_main, bg=self.bg_color, fg=self.fg_color, anchor='w').pack(fill=tk.X, pady=(10, 5))
    ...
    def load_model(self):
        filepath = filedialog.askopenfilename(filetypes=[("PyTorch Model", "*.pth")], title="artist_x_best_model.pth 파일을 선택하세요")
    ...
    def encode_text(self):
        ...
        with torch.no_grad():
            ...
            print(f"  - 토큰 ID: {inputs['input_ids'].tolist()[0][:original_length]}")
            print(f"  - 실제 토큰 길이: {original_length}")

            print("\n[인코딩 2단계: 토큰 -> 잠재 벡터]")
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
            latent_vector = self.model.encoder(inputs['input_ids'], attention_mask=inputs['attention_mask']).last_hidden_state
            print(f"  - 생성된 잠재 벡터 Shape: {latent_vector.shape}")
            print(f"  - 잠재 벡터 일부 (첫 토큰의 첫 5개 값): {latent_vector[0, 0, :5].tolist()}")
            
        # 💡 encode_data_to_image 함수는 내부적으로 로그를 출력하지 않도록 수정 (원할 경우 추가 가능)
        ...
        print(f"✅ 인코딩 완료. 이미지가 '{filepath}'에 저장되었습니다.")
        print("="*52)
    ...
    def decode_image(self):
        ...
        with torch.no_grad():
            ...
            print(f"  - 모델이 예측한 전체 토큰 ID (처음 {original_length+5}개): {predicted_ids.tolist()[0][:original_length+5]}")

            print("\n[디코딩 3단계: 예측 토큰 -> 최종 텍스트]")
            ...
            print(f"  - 길이 정보로 잘라낸 토큰 ID: {clean_ids.tolist()}")
            print(f"  - 최종 복원된 텍스트: '{reconstructed_text}'")
